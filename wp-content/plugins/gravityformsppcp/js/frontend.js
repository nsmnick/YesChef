...
/* global paypal, gf_check_field_rule */
...
(function ($) {
    ...
    window.GFPPCP = function (args) {
            ...
            gform.addAction('gform_frontend_feeds_evaluated', function (feeds, formId) {
                ...
                for (var i = 0; i < babel_runtime_core_js_object_keys__WEBPACK_IMPORTED_MODULE_4___default()(feeds).length; i++) {
                    if (feeds[i].addonSlug === 'gravityformsppcp' && feeds[i].isActivated) {
                        self.feedActivated = true;
                        var scriptTag = $('script[data-identifier="gform_ppcp_js_sdk"]');
                        var src = scriptTag.attr('src');
                        src = src.replace('&intent=authorize', '');
                        ...
                        for (var j = 0; j < babel_runtime_core_js_object_keys__WEBPACK_IMPORTED_MODULE_4___default()(self.feeds).length; j++) {
                            if (self.feeds[j].feedId === feeds[i].feedId) {
                                ...
                                if (self.feeds[j].intent === 'authorize') {
                                    src += '&intent=authorize';
                                }
        ...
        self.applyPriceClass = function () {
            ...
            var fieldsKey = babel_runtime_core_js_object_keys__WEBPACK_IMPORTED_MODULE_4___default()(window['gf_form_conditional_logic'][self.formId].fields);
            var fieldsValue = babel_runtime_core_js_object_values__WEBPACK_IMPORTED_MODULE_3___default()(window['gf_form_conditional_logic'][self.formId].fields);
            var fields = [];
            ...
            for (var _i = 0; _i < fields.length; _i++) {
                $('#field_' + self.formId + '_' + fields[_i]).not('.gfield_price').addClass('gform_ppcp_field_price');
            }
        };
            ...
            if (window['gf_coupons' + self.formId] !== undefined && self.activeFeed.paymentAmount === 'form_total') {
                ...
                for (var i = 0; i < size; i++) {
                    ...
                    if (discount.type === 'flat') {
                        // Discount amount is the same as coupon amount because it is a flat coupon.
                        discountTotal += gform.applyFilters('gform_coupons_discount_amount', discount.amount, discount.type, discount.amount, self.itemTotal, discountTotal);
                    }
                }
                // Calculate percentage discount AFTER discounted the flat amount.
                for (var _i2 = 0; _i2 < size; _i2++) {
                    discount = babel_runtime_core_js_object_values__WEBPACK_IMPORTED_MODULE_3___default()(window['gf_coupons' + self.formId])[_i2];
                    if (discount.type === 'percentage') {
                        var couponAmount = discount.amount;
                        var discountValue = gformRoundPrice((self.itemTotal + self.shipping - discountTotal) * couponAmount / 100);
                        discountTotal += gform.applyFilters('gform_coupons_discount_amount', discountValue, discount.type, couponAmount, self.itemTotal, discountTotal);
                    }
                }
            }
        ...
        self.getOrderData = function () {
            ...
            var pricingFields = $('.gform_ppcp_field_price :input, .gfield_price :input');
        ...
        /**
         * Determine whether the PayPal Smart Checkout buttons can be displayed.
         *
         * @return {boolean} Result.
         */
        self.canShowSmartButtons = function () {
            var canShowSubmitButton = gf_check_field_rule(self.formId, 0) === 'show';
            var canShowPayPalButtons = canShowSubmitButton && gf_check_field_rule(self.formId, self.ccFieldId) === 'show';

            return canShowPayPalButtons && self.feedActivated && window[self.totalName] && self.paymentMethod === 'PayPal Checkout' && self.isLastPage();
        };

        /**
         * Determine whether the Custom Card Field inputs and submit button can be displayed.
         *
         * @return {boolean} Result.
         */
        self.canShowCustomCardFields = function () {
            return self.paymentMethod === 'Credit Card' && paypal.HostedFields.isEligible() && window[self.totalName] > 0;
        };

        /**
         * Render the PayPal field, depending on the chosen payment method.
         */
        self.renderPayPalField = function () {
        ...
        self.renderPayPalButtons = function () {
            ...
            self.togglePaypalButtons('inline-block');

            if (self.paypalButtons.is(':empty')) {
        ...
        self.renderCustomCardFields = function () {
            if (!self.canShowCustomCardFields()) {
                $('.ginput_container_custom_card_fields').hide();
                return;
            }

            if (!$(self.form).data('gfppcpcustomcardrendered') && !self.requestingCards) {
                ...
                paypal.HostedFields.render({
                    ...
                    $(self.form).attr('data-gfppcpcustomcardrendered', true);
                    self.requestingCards = false;

                    hf.on('focus', function (event) {
                        if (event.emittedBy === 'number') {
                            wp.a11y.speak($('#field_' + self.formId + '_' + self.ccFieldId + '_supported_creditcards').text());
                        }
                    });
        ...
        self.togglePaypalButtons = function (status) {
            $(self.paypalButtons).css('display', status);
        };
        ...
        self.displayCardError = function (message) {
            ...
            var messageContainer = self.GFCCField;
            // If field is hidden when checkout is the only available option, use button container as message container.
            if ($(messageContainer).is(':hidden')) {
                messageContainer = '#gform_ppcp_smart_payment_buttons';
            }

            if (!$(messageContainer).find('.validation_message').length) {
                $(messageContainer).append('<div class="gfield_description validation_message"></div>');
            }

            var cardErrors = $(messageContainer).find('.validation_message');
