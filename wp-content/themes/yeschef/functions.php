...
add_filter( 'gform_pre_render_5', 'my_populate_promotion2' );
function my_populate_promotion2( $form ) {
    ...
    if ( $the_query->found_posts > 0 ) {
      ...
      while ( $the_query->have_posts() ) : $the_query->the_post();
        ...
        $meal_choices[] = array( 'text' => $title, 'value' => $title, 'price'=>$price );
    ...
    if($promotion_active)
    {
      ...
      $order_option_text_for_4_people_caption = $order_option_text_for_4_people. '<span class="radio-lb">' . get_field('order_option_text_for_2_people','option') . '</span>';
...
add_filter( 'gform_pre_render_5', 'my_populate_orderform4' );
function my_populate_orderform4( $form ) {
...
//
// Publish Fields and Update form fields
//



add_action( 'publish_nsm_meals', 'post_published_nsm_meals', 100, 2 );
function post_published_nsm_meals($postid)
{
  ...
  $pie_option = get_post_meta(get_the_ID(), 'lazy_day_pie_option');
  $supplement_price = get_post_meta(get_the_ID(), 'lay_day_pie_option_price', true);
